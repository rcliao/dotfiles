...
# Skip trivial sessions: require real work (>= 2 tool calls), not raw line count.
# JSONL preamble (system reminders, etc.) alone can clear a naive line threshold.
TOOL_USE_COUNT=$(grep -c '"type":"tool_use"' "$TRANSCRIPT_PATH" 2>/dev/null || true)
TOOL_USE_COUNT=${TOOL_USE_COUNT:-0}
if [ "$TOOL_USE_COUNT" -lt 2 ]; then
  echo "Trivial session ($TOOL_USE_COUNT tool calls), skipping" >>"$DEBUG_LOG"
  exit 0
fi

# Debounce: the Stop hook fires after EVERY assistant turn. Without this, long
# auto-mode sessions re-run the claude -p extraction and re-mint memory keys on
# every turn, polluting the store and burning a model call each time. Only
# re-capture once the transcript has grown by >= GROWTH_THRESHOLD lines since the
# last capture for this session. Final turns still get captured as the file grows.
GROWTH_THRESHOLD="${GHOST_STOP_GROWTH:-80}"
STATE_DIR="/tmp/ghost-stop-state"
...
if [ "$((CUR_LINES - LAST_LINES))" -lt "$GROWTH_THRESHOLD" ]; then
  echo "Debounce: $((CUR_LINES - LAST_LINES)) new lines (<$GROWTH_THRESHOLD) since last capture, skipping" >>"$DEBUG_LOG"
  exit 0
fi
# Advance the watermark immediately so concurrent Stop fires during the slow
# claude -p call below don't all proceed.
...
TRANSCRIPT_SAMPLE=$(
  head -30 "$TRANSCRIPT_PATH" 2>/dev/null
  echo "---"
  tail -70 "$TRANSCRIPT_PATH" 2>/dev/null
)
...
echo "$LEARNINGS" | jq -c '.[]' | while IFS= read -r item; do
  ...
  if PUT_OUTPUT=$($GHOST put -n "$AGENT_NS" -k "$KEY" --kind "$KIND" -p "$PRIORITY" --tier "$TIER" -t "$TAGS" --dedup "$CONTENT" 2>>"$DEBUG_LOG") && [ -n "$PUT_OUTPUT" ]; then
